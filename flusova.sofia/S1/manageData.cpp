...
void flusova::manageExpression::parseExpression(const std::string& input) {
  std::string token;
  std::string number;
  for (std::size_t i = 0; i < input.length(); i++) {
    token = input[i];
    flusova::Token pushToken(const_cast<char*>(token.c_str()));
    if (i == input.length() - 1) {
      if (!number.empty() && pushToken.getTokenType() == flusova::OPERAND) {
        number += input[i];
        flusova::Token pushNumber(const_cast<char*>(number.c_str()));
        newExpr_.pushFront(pushNumber);
      }
      else if (pushToken.getTokenType() == flusova::OPERAND) {
    ...
    if (token == " ") {
      if (!number.empty()) {
        if (number.length() == 19 && number[18] == '8') {
          std::cerr << "Overflow!" << '\n';
          exit(1);
        }
        flusova::Token pushNumber(const_cast<char*>(number.c_str()));
    ...
    if (pushToken.getTokenType() == flusova::OPERAND) {
      number += token;
      continue;
    }
    newExpr_.pushFront(pushToken);
...
short flusova::manageExpression::checkOperatorsPriority(flusova::Token token) const {
  while (true)
  {
...
void flusova::manageExpression::convertToPolish(const std::string& newString) {
...
void flusova::manageExpression::calculatePolish(flusova::Stack< long long > &stackResult) {
...
void flusova::manageExpression::printResult(flusova::Stack< long long >& stackResult)
{
